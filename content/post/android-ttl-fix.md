...
Tags = ["android"]
date = "2019-02-19T15:35:14+03:00"
title = "Фиксация TTL на Android"
...
Теперь идем сюда - https://android.googlesource.com/platform/prebuilts/gcc/linux-x86/
...
Указываем архитектуру и путь до бинарей тулчейна:
{{< highlight console >}}
$ export ARCH=arm64
...
Теперь надо создать конфиг для будущей сборки. Возможные варианты конфигов лежат в папке с исходниками ядра в ```arch/arm64/configs``` (вместо arm64 может быть ваша архитектура). Вспоминая версию ядра, которая установлена в телефоне, выбираю конфиг ```msm-perf_defconfig```:
{{< highlight console >}}
...
$ make modules
{{< /highlight >}}
Если эти команды завершились ошибкой, значит, скорее всего неправильно выбран toolchain, конфиг или сами исходники ядра. Определенные исходники можно собрать только подходящим тулчейном и с подходящим конфигом.
...
{{< /highlight >}}
Если insmod возвращает ошибку ```exec format error```, значит, скорее всего, неправильно были выбрана версия исходников ядра или архитектура. В ```dmesg``` можно узнать подробности ошибки совместимости и пересобрать под правильную версию.
...
В конце добавляем в автозагрузку телефона скрипт, который будет включать фиксацию TTL:
{{< highlight bash >}}
#!/system/bin/sh
...
Такой скрипт я положил в ```/sdcard/fixttl.sh```, и с помощью приложения [ScriptManager](https://play.google.com/store/apps/details?id=os.tools.scriptmanager), добавил в атозагрузку. 

![ScriptManager](https://habrastorage.org/files/515/7d9/79e/5157d979eab84cc688c56fc1a4b9b6a1.png)
